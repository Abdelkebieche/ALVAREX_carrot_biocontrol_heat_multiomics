...
TIME_SHAPES <- c("P2" = 16, "P3" = 17)
TIME_LABELS <- c("P2" = "D2", "P3" = "D4")
...
expr_mat <- readRDS(file.path(run_dir, "expr_mat_VST_MAD.rds"))
...
cat("  Samples:", nrow(meta), "| Genes:", ncol(datExpr), "\n\n")
...
p_temp <- ggplot2::ggplot() +
  ...
  ggplot2::labs(title = paste0("PCA — ", GENO, " (D2+D4)"),
...
# ── PCA Vue 2: Traitement (EAU vs SDP) ──
...
p_trt <- ggplot2::ggplot() +
  ...
  ggplot2::labs(title = paste0("PCA — ", GENO, " | Control vs Treated par température"),
...
# ── PCA Vue 3: Time point (P2 vs P3) ──
p_time <- ggplot2::ggplot() +
  ggplot2::geom_point(data = pca_df,
                      ggplot2::aes(PC1, PC2, colour = Sampling_time, shape = Treatment),
                      alpha = 0.40, size = 2) +
  ggplot2::facet_wrap(~ Temperature, labeller = ggplot2::labeller(Temperature = c("T0"="NH","T1"="HS-7","T2"="HS-2")), nrow = 1) +
  ggplot2::scale_colour_manual(values = TIME_COLORS, labels = c("P2"="D2","P3"="D4")) +
  ggplot2::scale_shape_manual(values = TRT_SHAPES) +
  ggplot2::labs(title = paste0("PCA — ", GENO, " | D2 vs D4 par température"),
                x = xlab, y = ylab) +
  theme_pub()
...
ylab34 <- paste0("PC3 (", pct[3], "%)")
xlab34 <- paste0("PC2 (", pct[2], "%)")
...
p_multi <- p_pc12 + p_pc34 + patchwork::plot_layout(guides = "collect") +
  patchwork::plot_annotation(title = paste0(GENO, " — Multi-PC view (D2+D4)"))
...
save_base_plot(function(){
  WGCNA::plotDendroAndColors(
    ...
    main = paste0("Sample clustering — ", GENO, " (D2+D4)"),
    cex.dendroLabels = 0.6, cex.colorLabels = 0.8
  )
}, file.path(dir_qc, "Sample_dendrogram"), w = 14, h = 6)

##############################
## PARTIE 3: Heatmap top genes
##############################
if (HAS_PHEATMAP) {
  cat("── HEATMAP TOP GENES ──\n")
  ...
  ph <- pheatmap::pheatmap(
    ...
    border_color     = NA,
    fontsize         = 8,
    main = paste0("Top ", n_hm, " variable genes (z-score) — ", GENO),
    silent = TRUE
  )
  
  pdf(file.path(dir_qc, "Heatmap_top_genes.pdf"), width = 10, height = 9)
  grid::grid.draw(ph$gtable)
  dev.off()
  
  png(file.path(dir_qc, "Heatmap_top_genes.png"), width = 10, height = 9,
      units = "in", res = 300, bg = "white")
  grid::grid.draw(ph$gtable)
  dev.off()
  
  svglite::svglite(file.path(dir_qc, "Heatmap_top_genes.svg"), width = 10, height = 9, bg = "white")
  grid::grid.draw(ph$gtable)
  dev.off()
}
