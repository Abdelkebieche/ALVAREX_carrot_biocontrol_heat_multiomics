...
softPower <- if (GENO %in% names(RNA_SOFT_POWER)) unname(RNA_SOFT_POWER[[GENO]]) else softPower_auto
log_msg("Publication soft power: ", softPower, " | diagnostic auto-choice: ", softPower_auto)
...
cat("  Forcing single block: nGenes =", ncol(datExpr), "\n")
...
write.csv(gene_mod, file.path(dir_net, "Gene_to_Module.csv"), row.names = FALSE)
...
if (!is.null(net$dendrograms) && length(net$dendrograms) > 0) {
  save_base_plot(function(){
    WGCNA::plotDendroAndColors(
      ...
      main = paste0("Gene dendrogram — ", GENO, " | β=", softPower)
    )
  }, file.path(dir_net, "Gene_dendrogram_modules"), w = 14, h = 6)
}
...
mod_sizes <- as.data.frame(table(colors)) |>
  ...
  dplyr::mutate(Module = forcats::fct_reorder(Module, N))

if (!requireNamespace("forcats", quietly = TRUE)) {
  mod_sizes$Module <- factor(mod_sizes$Module, levels = mod_sizes$Module[order(mod_sizes$N)])
}
...
p_sizes <- ggplot2::ggplot(mod_sizes, ggplot2::aes(Module, N, fill = as.character(Module))) +
  ...
  ggplot2::coord_flip() +
  ggplot2::labs(title = paste0("Module sizes — ", GENO),
