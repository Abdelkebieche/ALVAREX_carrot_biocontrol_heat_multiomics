############################################################
## 04_module_trait.R — MODULE-TRAIT ANALYSIS
## ─────────────────────────────────────────────────────────
## LE SCRIPT PRINCIPAL: répond aux questions biologiques
##
## 1. Heatmap ME × ALL traits (style "protection")
...
## 5. Boxplots ME par condition (chaque condition séparément)
...
log_msg("START Script 04 — Module-Trait")
...
##############################################################
## PARTIE 1: MATRICE DE TRAITS BINAIRES (style protection)
##############################################################
cat("── PARTIE 1: Matrice de traits ──\n")

# 1A) Indicatrices simples (chaque condition seule)
traits <- data.frame(
  # Températures
...
x_P3  <- ifelse(meta$Sampling_time == "P3", 1, ifelse(meta$Sampling_time == "P2", -1, NA))
...
# Fonction bicor robuste pour colonnes avec NA
...
##############################################################
## PARTIE 3: HEATMAP ME × ALL TRAITS (style protection)
##############################################################
cat("── PARTIE 3: Heatmaps ──\n")

# 3A) Heatmap: conditions simples (T0,T1,T2,EAU,SDP,P2,P3)
simple_cols <- c("T0","T1","T2","Control","Treated","P2","P3")
simple_cols_display <- c("NH","HS-7","HS-2","Control","Treated","D2","D4")
...
save_base_plot(function(){
  ...
  WGCNA::labeledHeatmap(
    ...
    xLabelsAngle = 45,
    main = paste0("ME vs conditions — ", GENO, " (D2+D4)\nbicor | *FDR<0.05 **<0.01 ***<0.001")
  )
}, file.path(dir_mt, "Heatmap_ME_vs_Conditions"), w = 10,
h = max(8, nrow(cor_mat) * 0.30 + 3))

# 3B) Heatmap: contrastes (SDP_in_T0, SDP_in_T1, SDP_in_T2, T1vsT0, T2vsT0, P3vsP2)
contrast_cols <- c("SDP_in_T0","SDP_in_T1","SDP_in_T2","T1_vs_T0","T2_vs_T0","P3_vs_P2")
contrast_cols_display <- c("Treated vs Control (NH)","Treated vs Control (HS-7)","Treated vs Control (HS-2)","HS-7 vs NH","HS-2 vs NH","D4 vs D2")
...
if (length(contrast_cols) > 0) {
  ...
  save_base_plot(function(){
    ...
    WGCNA::labeledHeatmap(
      ...
      cex.text = 0.55, zlim = c(-1, 1),
      xLabelsAngle = 45,
      main = paste0("ME vs effects — ", GENO, " (D2+D4)\nbicor | *FDR<0.05")
    )
  }, file.path(dir_mt, "Heatmap_ME_vs_Effects"), w = 10,
  h = max(8, nrow(cor_mat) * 0.30 + 3))
}

# 3C) Heatmap: Treated effect par T° et par Time
...
# Display labels for detailed heatmap
...
if (length(detailed_cols) > 2) {
  ...
  save_base_plot(function(){
    ...
    WGCNA::labeledHeatmap(
      ...
      xLabelsAngle = 60,
      main = paste0("ME vs detailed effects — ", GENO, "\nTreated vs Control per T° | T° per Treatment | D4 vs D2")
    )
...
##############################################################
## PARTIE 4: ANOVA — modèle linéaire par module
##############################################################
cat("── PARTIE 4: ANOVA ──\n")
...
for (me in colnames(MEs_clean)) {
  ...
  # Modèle: ME ~ Temperature * Treatment + Time
  fit <- tryCatch(
  ...
  # ANOVA type II (car::Anova si dispo, sinon anova)
  a <- tryCatch({
...
colnames(anova_pmat) <- c("Temperature", "Treatment", "Time (P2/P3)", "Temp × Trt")
...
##############################################################
## PARTIE 5: BOXPLOTS — ME par condition (chaque condition)
##############################################################
cat("── PARTIE 5: Boxplots ──\n")

# Modules à plotter: tous ceux avec au moins 1 facteur significatif
...
for (i in seq_len(nrow(sig_any))) {
  ...
  # 5D) Boxplot SDP vs EAU dans chaque T° (LE plus informatif)
  p_sdp <- ggplot2::ggplot(df, ggplot2::aes(Treatment, ME, fill = Treatment)) +
    ...
    ggplot2::facet_wrap(~ Temperature, labeller = labeller(Temperature = c("T0"="NH","T1"="HS-7","T2"="HS-2")), nrow = 1) +
    ggplot2::scale_fill_manual(values = TRT_COLORS) +
  ...
  p_combo <- (p_top / p_sdp / p_full) +
    patchwork::plot_annotation(
      title = paste0("Module ", colr, " (", me, ") — ", GENO),
      subtitle = sub_txt,
      theme = ggplot2::theme(
...
##############################################################
## PARTIE 6: TESTS PAIRÉS — Wilcoxon SDP vs EAU par T° et Time
##############################################################
...
classify <- anova_results |>
  dplyr::mutate(
    ...
    Classification = dplyr::case_when(
      ...
      Time_responsive ~ "Time-responsive (P2→P3)",
      TRUE ~ "Stable"
    )
  ) |>
...
p_class <- ggplot2::ggplot(class_count,
  ...
  ggplot2::coord_flip() +
  ggplot2::labs(title = paste0("Module classification — ", GENO),
